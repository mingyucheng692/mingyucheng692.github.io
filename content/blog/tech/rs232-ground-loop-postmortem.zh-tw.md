...
工控機可經 SSH 登入 Linux，無需盲測。核心串列埠驅動自帶免費觀測點。
...
雙向明文恢復，fe 計數不再增長。換用雙隔離轉接器後插電場景同樣穩定——問題閉環。
...
- `/proc/tty/driver/serial` 的 tx/rx/fe 計數器是核心級免費觀測點：rx 增長證明實體送達，fe 不增長證明訊框完整——比序列埠工具直觀可靠
...
- 首位元組丟失（`HELLO_PC` → `ELLO_PC`）是 UART 起始位元同步抖動，連續通訊即恢復，不是故障
