...
在一次面向设备接入与 Web 应用认证链路的复现测试中，我们重点模拟了“旧 RT 被拦截并重复提交”的极端情况。  
...
定期执行“RT 重放”验证，持续检查告警、失效和回收链路是否生效。

---

## 后续可选增强项

当前方案已经完成 Redis 会话撤销链路与并发防重放机制。Refresh 流程会实时校验用户状态，账号进入锁定或禁用状态时会即时阻断新票据签发。

如果后续需要覆盖更复杂的 Token 劫持场景，可继续增加以下控制：

- 环境指纹对比：可引入 IP 段与 UA Hash 组合校验，在 RT 出现异常地域跳变时触发更严格的票据失效与附加验证。
- 规则校正：在复杂网络环境下，需先验证 4G/5G 频繁切网、出口漂移等合法抖动，再决定是否启用更严格的判定规则。

---

## 结语

认证安全的关注点之一，是异常发生后的收口能力。  
这次改造补上了 RT 刷新链路中的会话撤销与并发防重放控制。
