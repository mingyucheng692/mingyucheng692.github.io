...
summary: "面向嵌入式開發與現場聯調的輕量級 Modbus 工具。本文以實作流程為主，重點介紹如何快速發幀、查看並複製日誌，以及使用幀解析器完成倍率換算、寄存器註解與配置持久化。"
...
開發 Modbus-Tools 的目標很明確：把「發幀、看日誌、解幀」三件高頻操作做順手。它不追求功能堆疊，而是以調試效率和可用性為優先。
...
配置可保存並重複利用。
- **自動保存**：保留最近使用的倍率與註解。
- **JSON 匯入/匯出**：可依設備建立專屬模板，切換機型時直接套用。
