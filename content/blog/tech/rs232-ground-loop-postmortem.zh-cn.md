...
工控机可经 SSH 登录 Linux,无需盲测。内核串口驱动自带免费观测点。
...
双向明文恢复，fe 计数不再增长。换用双隔离转换器后插电场景同样稳定——问题闭环。
...
- `/proc/tty/driver/serial` 的 tx/rx/fe 计数器是内核级免费观测点：rx 增长证明物理送达，fe 不增长证明帧完整——比串口助手直观可靠
...
- 首字节丢失（`HELLO_PC` → `ELLO_PC`）是 UART 起始位同步抖动，连续通信即恢复，不是故障
